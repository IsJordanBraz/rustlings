...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        scores.entry(team_1_name.clone()).and_modify(|t| { t.add(team_2_score, team_1_score);})
        .or_insert(Team{goals_scored: team_1_score, goals_conceded: team_2_score});

        scores.entry(team_2_name.clone()).and_modify(|t| { t.add(team_1_score, team_2_score);})
